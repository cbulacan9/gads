"""
Hyper3D Rodin Tool for GADS

AI-powered 3D model generation using Hyper3D Rodin via Blender MCP addon.

ARCHIVED: This code was extracted from blender_mcp.py and removed from GADS core.
The Hyper3D integration requires Claude's MCP context to function, which doesn't
fit well with GADS's standalone CLI architecture.
...
3. Valid Hyper3D API key configured (or fal.ai account)
"""
